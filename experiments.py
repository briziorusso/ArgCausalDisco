...
parser.add_argument('--source', choices=['causenet', 'bnlearn'], default='causenet', help='Dataset source')
...
parser.add_argument('--return_statistics', type=str_to_bool, default=False, metavar='{true,false}', help='ABAPC: request solver statistics when supported')
        ...
        for idx in range(completed_runs, n_runs):
            ...
            if src == 'causenet':
                X_s, B_true = load_causenet_data_dag(
                    ...
                    standardise=(False if standardise is None else standardise),
                    simulate_with=simulate_with,
                )
            else:
                # BNLearn default standardisation True unless overridden by flag
