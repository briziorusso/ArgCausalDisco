...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--seeds", nargs="+", type=int, help="Explicit seed identities, overriding the contiguous range")
    ...
    try:
        args.n_runs = len(runner.selected_seeds(args))
...
def _regeneration_args(metadata: dict[str, Any]) -> SimpleNamespace:
    return SimpleNamespace(
        ...
        aij_inputs=bool(metadata.get("aij_inputs",False)),
...
def validate(args: argparse.Namespace) -> dict[str, Any]:
    ...
    expected_seeds = runner.selected_seeds(args)
