...
from scripts.runners.run_bnlearn_abapc_orig_matched10 import (
    MATCHED_DIR,
    DEFAULT_PYTHON,
    build_command,
    canonical_seed_list,
...
DEFAULT_NAMES = ["asia", "survey"]


def main() -> None:
    parser = argparse.ArgumentParser(description="Run ABAPC (orig) on asia/survey using matched gsq facts.")
    parser.add_argument('--version', default='abapc_orig_problem3_matched10_gsq')
    ...
    note = {
        ...
        'note': 'ABAPC (orig) comparison runner for asia/survey on matched gsq facts; sachs excluded because it is too expensive.'
    }
