...
PAPER_DATASETS = ("cancer", "earthquake", "survey", "asia", "sachs", "child", "er5", "er8", "sf5", "sf8")
...
from scripts.run_fgs_once import FGS_BDEU_CONFIG
...
def selected_seeds(args: argparse.Namespace) -> list[int]:
    """Explicit identities take precedence over the legacy contiguous range."""
    seeds = getattr(args, "seeds", None)
    seeds = list(seeds) if seeds is not None else list(range(args.seed_start, args.seed_start + args.n_runs))
...
def _run_fgs_bdeu(X: np.ndarray) -> tuple[np.ndarray, float]:
    """Isolate each JVM lifetime and preserve native Tetrad endpoints."""
    with tempfile.TemporaryDirectory(prefix='aij_fges_') as temporary:
        directory=Path(temporary)
        data,graph,meta=(directory/name for name in ('data.npy','graph.npy','meta.json'))
        np.save(data,X)
        subprocess.run([sys.executable,str(REPO_ROOT/'scripts/run_fgs_once.py'),
                        '--input',str(data),'--output',str(graph),'--meta',str(meta),
                        '--score','bdeu-score'],check=True)
        info=json.loads(meta.read_text())
        if not info['ok']:
            raise RuntimeError(info.get('error','FGS-BDeu failed'))
        return np.load(graph),float(info['search_elapsed'])
...
def _dataset_specs(names: list[str]) -> list[DatasetSpec]:
    ...
    for name in names:
        ...
        elif n in {"sachs", "child"}:
            specs.append(DatasetSpec(n, "bnlearn", 11 if n=="sachs" else 20))
        elif n in {"er5", "er_5"}:
...
def _load_data(spec: DatasetSpec, *, sample_size: int, seed: int, args: argparse.Namespace):
    if spec.source=='bnlearn' and getattr(args,'aij_inputs',False):
        if sample_size!=5000 or not args.bn_standardise:
            raise ValueError('AIJ archived inputs require n=5000 and standardised categories')
        data=REPO_ROOT/f'datasets/data_npy/data_{spec.name}_{seed}.npy'
        truth=REPO_ROOT/f'datasets/data_npy/true_graph_{spec.name}_{seed}.npy'
        if data.exists() and truth.exists():
            X,B=np.load(data),np.load(truth)
            if X.shape!=(sample_size,spec.n_nodes) or B.shape!=(spec.n_nodes,spec.n_nodes):
                raise ValueError(f'Unexpected archived input shape: {data}')
            return X,B
...
def _compute_metrics(W_est: np.ndarray, B_true: np.ndarray, method=None) -> tuple[dict[str, Any], dict[str, Any]]:
    from utils.aij_graph_metrics import prepare_estimate
    from utils.graph_utils import DAGMetrics
    key = 'fgs' if method == 'fgs_bdeu' else method
    dag, cpdag = prepare_estimate(W_est, key)
    return (DAGMetrics(dag, np.asarray(B_true), sid=False, evaluation_kind='dag').metrics,
            DAGMetrics(cpdag, np.asarray(B_true), sid=False, evaluation_kind='cpdag').metrics)
...
def _metadata(args: argparse.Namespace, specs: list[DatasetSpec], out_dir: Path) -> dict[str, Any]:
    return {
        ...
        "graph_metric_protocol": "aij_v2",
        "aij_inputs": getattr(args,"aij_inputs",False),
        ...
        "n_runs": len(selected_seeds(args)),
...
def _assert_metadata_compatible(existing: dict[str, Any], proposed: dict[str, Any]) -> None:
    identity_fields = [
        "graph_metric_protocol", "aij_inputs", "version", "methods", "aspcr_method_configs", "fgs_bdeu_config", "datasets", "n_runs", "seeds",
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--seeds", nargs="+", type=int, help="Explicit seed identities, overriding --seed-start and --n-runs")
    ...
    parser.add_argument("--aij-inputs", action="store_true", help="Reuse archived matched AIJ input arrays where available")
    ...
    try:
        args.n_runs = len(selected_seeds(args))
        ...
        for method in args.methods:
            ...
            for run_idx, seed in enumerate(seeds):
                ...
                try:
                    ...
                    dag_metrics, cpdag_metrics = _compute_metrics(np.asarray(W_est), np.asarray(B_true), method=method)
