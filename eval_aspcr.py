...
for dataset_name in dataset_list:
    names_dict = {'pc':'PC', 'pc_max':'Max-PC', 'fgs':'FGS', 'spc':'Shapley-PC', 'mpc':'MPC', 'cpc':'CPC', 'abapc':'ABAPC (Ours)', 
                  'cam':'CAM', 'nt':'NOTEARS-MLP', 'mcsl':'MCSL-MLP', 'ges':'GES', 'random':'Random', 'aspcr_log':'ASPCR-log', 
                  'aspcr_hard':'ASPCR-hard', 'aspcr_const':'ASPCR-const', 'aspcr_test':'ASPCR-test'}
    ...
    for method in model_list:
        ...
        for seed in seeds_list:
            ...
            if save_data:
                pd.DataFrame(B_true).to_csv(f"results/aspcr/data/true_graph_{dataset_name}_{seed}.csv", index=False, header=False)
                pd.DataFrame(X_s).to_csv(f"results/aspcr/data/data_{dataset_name}_{seed}.csv", index=False, header=False)
        ...
        if 'aspcr' in method and save_data:
            continue  
