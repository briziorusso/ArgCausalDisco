"""
From CausalLearn, a Python package for causal structure learning, we added error exceptions to the FisherZ class.
"""
...
#### Add fallback options when correlation matrix calculation fails
class FisherZ(CIT_Base):
    ...
    def __call__(self, X, Y, condition_set=None):
        ...
        try:
            ...
            r = 0 # when cannot calculate the correlation coefficient = cannot reject null hypothesis
        try:
            Z = 0.5 * log((1 + r) / (1 - r))
        except ValueError:
            Z = 0 # when cannot calculate the Fisher-Z score = cannot reject null hypothesis
